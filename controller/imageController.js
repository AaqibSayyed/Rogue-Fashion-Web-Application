...
async function imageUploadController(req, res) {
    let check_files = await multer_helper.parseFile(req, res, {
        size: 4000 * 1000,
        ext: /jpeg|jpg|png/,
        field: [{ name: "product_image", maxCount: 1 }, { name: "single_product_image", maxCount: 1 }]
    }).catch((error) => { return { error } })
    // console.log('i am from check files', check_files)
    if (!check_files || (check_files && check_files.error)) {
        return res.status(check_files.status).send({ error: check_files.error })
    }
    ...
    // console.log('upload_image_check', upload_image_check)

    if (!upload_image_check || (upload_image_check && upload_image_check.error)) {
        return res.status(upload_image_check.status).send({ error: upload_image_check.error })
    }

    return res.status(upload_image_check.status).send({ data: upload_image_check.data })
